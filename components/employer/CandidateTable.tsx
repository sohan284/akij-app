...
export default function CandidateTable({ exam, initialCandidates }: CandidateTableProps) {
  ...
  const handleAddCandidate = async () => {
    ...
    try {
      const response = await axios.post(`/api/tests/${exam.id}/candidates`, newCandidate);
  ...
  return (
    <div className="flex flex-col flex-1">
      {/* Back Link */}
      <Link href="/employer/dashboard" className="flex items-center text-sm font-medium text-slate-500 hover:text-primary transition-colors mb-6 group w-fit">
